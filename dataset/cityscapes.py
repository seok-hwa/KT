...
class CSTrainValSet(data.Dataset):
    ...
    def generate_scale_label(self, image, label):
        ...
        # f_scale = 0.5 + random.randint(0, 11) / 10.0 #skd
        # f_scale = 0.7 + random.randint(0, 14) / 10.0 #cwd
...
class CSValSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(512, 1024), ignore_label=-1):
        ...
        for item in self.img_ids:
            ...
            label_file = osp.join(self.root, label_path)
            # image_path = item
            # tmp_name_list = image_path[0].split('/')
            # tmp_name_list[0] = 'gtFine'
            # tmp_img_name = tmp_name_list[-1]
            # tmp_img_name_list = tmp_img_name.split('_')
            # tmp_img_name_list[-1] = 'gtFine_labelIds.png'
            # tmp_img_name = ('_').join(tmp_img_name_list)
            # tmp_name_list[-1] = tmp_img_name
            # label_path = ('/').join(tmp_name_list)
...
class CSTestSet(data.Dataset):
    def __init__(self, root, list_path):
        ...
        for item in self.img_ids:
            ...
            # img_file = osp.join(self.root, image_path)
            # print(image_path)
            tmp_name_list = image_path.split('/')
            tmp_name_list[0] = 'gtFine'
            tmp_img_name = tmp_name_list[-1]
            tmp_img_name_list = tmp_img_name.split('_')
            tmp_img_name_list[-1] = 'gtFine_labelIds.png'
